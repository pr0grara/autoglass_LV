# Vegas Mobile Auto Glass — Lead-Gen Site

A static, config-driven, SEO-first lead-gen website for **mobile auto glass & power window repair
across Las Vegas**. Built from [`../SPEC.md`](../SPEC.md) (the single source of truth) using Astro 5
— no database, no server runtime, no client framework. Output is plain static HTML/CSS for a CDN.

> **Status:** placeholder business identity (phone `(702) 555-0198`, email, domain) + placeholder
> illustration images. It builds clean and is ready to launch once you swap in the real business
> details, real job photos, and a form backend — **do not edit `src/pages`, `src/layouts`,
> `src/components`, or `src/lib` to rebrand.** The answer is almost always in `src/config/`.
>
> No reviews, ratings, address, or licensed/insured claims are asserted — those are intentionally
> left out until they can be confirmed for the real business (see `Footer.astro` placeholder note).
