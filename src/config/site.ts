...
export interface SiteConfig {
  company: string;
  tagline: string;
  /** What the business does, lowercase, for prose: "auto glass repair" */
  trade: string;
  phone: string; // dialable, E.164
  phoneDisplay: string; // shown to visitors
  email: string;
  /** Service-area region name shown in headlines, e.g. "Las Vegas" */
  region: string;
...
export const SITE: SiteConfig = {
  ...
  email: 'service@vegasmobileautoglass.com', // PLACEHOLDER local-part — confirm the real inbox
  region: 'Las Vegas',
  url: 'https://vegasmobileautoglass.com', // real domain — keep in sync with astro.config.mjs
